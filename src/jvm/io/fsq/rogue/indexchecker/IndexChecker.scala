...
class MongoIndexChecker extends IndexChecker {
  ...
  override def validateQueryMatchesSomeIndex(query: Query[_, _, _], indexes: Seq[UntypedMongoIndex]) = {
    ...
    conditions.forall(clauses => {
      clauses.isEmpty || matchesUniqueIndex(clauses) ||
        ...
        signalError(query, "Query does not match an index! query: %s, indexes: %s" format (query.toString, indexString))
    })
  }
  ...
  private def matchesIndex(index: List[String], clauses: List[QueryClause[_]]) = {
  ...
  /**
    ...
    */
  private def matchesCompoundIndex(index: List[String], clauses: List[QueryClause[_]], scanning: Boolean): Boolean = {
    if (clauses.isEmpty) {
