...
class ConcreteBlamer extends Blamer with Logger {
  val blamer = Config
    .opt(_.getString("git.repo"))
    .map(r => new File(r))
    .find(f => f.exists && f.isDirectory)
    .map(rf => new GitBlamer(rf))
    .getOrElse({
  ...
  def blame(tag: String, fileName: String, line: Int, hints: Set[String]): Future[Option[Blame]] =
    blamer.blame(tag, fileName, line, hints)
...
class GitBlamer(val root: File) extends Blamer with Logger {
  ...
  def findFile(tag: String, name: String, hints: Set[String]): Future[Option[String]] = {
    Option(recentTags.get(tag))
      ...
      .getOrElse({
        ...
        val rescued: Future[ProcessResult] = result.rescue {
          case e => {
  ...
  def blame(tag: String, fileName: String, line: Int, hints: Set[String]): Future[Option[Blame]] = {
    ...
    findFile(tag, fileName, hints).flatMap { fileOpt =>
      fileOpt
        .map(f => {
          val gblame = retry(
          ...
          gblame.map(
            pr =>
              Some(pr.out.foldLeft(Blame(f, line)) { (b, l) =>
                updateBlame(l, b)
              })
          )
        })
