...
class SignificantKeywordsFilter extends KeywordFilter {
  val keywordProcessors = Config
    .opt(_.getStringList("incoming.keywordFields").asScala)
    .getOrElse(List("msgs", "excs", "bt"))
    .map(f => f -> RollingRank(f))
    .toMap

  def keywords(incoming: FilteredIncoming): Set[String] = {
    keywordProcessors.toList
      .map({
        case (field, ranker) => {
