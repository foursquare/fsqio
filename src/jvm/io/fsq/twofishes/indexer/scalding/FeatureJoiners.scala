...
object FeatureJoiners {
  ...
  def boundingBoxJoiner(
    ...
    features
      .leftJoin(bboxes)
      .map({
        case (k: LongWritable, (f: GeocodeServingFeature, bboxOpt: Option[GeocodeBoundingBox])) => {
          bboxOpt match {
            case Some(bbox) =>
  ...
  def displayBoundingBoxJoiner(
    ...
    features
      .leftJoin(displayBboxes)
      .map({
        case (k: LongWritable, (f: GeocodeServingFeature, displayBboxOpt: Option[GeocodeBoundingBox])) => {
          displayBboxOpt match {
            case Some(displayBbox) =>
              (k -> f
                .copy(feature = f.feature.copy(geometry = f.feature.geometryOrThrow.copy(displayBounds = displayBbox))))
            case None =>
              (k -> f)
          }
        }
  ...
  def extraRelationsJoiner(
    ...
    features
      .leftJoin(extraRelations)
      .map({
        case (k: LongWritable, (f: GeocodeServingFeature, containerOpt: Option[IntermediateDataContainer])) => {
          containerOpt match {
            case Some(container) =>
          ...
          containerOpt match {
            case Some(container) => {
              ...
              if (sortedSlugEntries.nonEmpty) {
                (k -> f.copy(
                  slugs = sortedSlugEntries.map(_.id),
                  feature = f.feature.copy(slug = sortedSlugEntries.head.id)
                ))
              } else {
                (k -> f)
              }
  ...
  def featureEditsJoiner(
    ...
    features
      .leftJoin(edits)
      .flatMap({
        case (k: LongWritable, (f: GeocodeServingFeature, editsOpt: Option[GeocodeServingFeatureEdits])) => {
          editsOpt match {
            case Some(edits) => {
  ...
  def polygonsJoiner(
    ...
    features
      .leftJoin(polygons)
      .flatMap({
        case (k: LongWritable, (f: GeocodeServingFeature, polygonOpt: Option[PolygonMatchingValue])) => {
          polygonOpt match {
            case Some(polygon) => {
              ...
              for {
                ...
                (k -> f.copy(
                  ...
                  feature = f.feature.copy(
                    geometry = f.feature.geometryOrThrow
                      .copy(wkbGeometry = ByteBuffer.wrap(wkbGeometry), bounds = bounds, source = source)
                  )
                ))
              }
            }
  ...
  def attributesJoiner(
    ...
    features
      .leftJoin(attributes)
      .map({
        case (k: LongWritable, (f: GeocodeServingFeature, attrOpt: Option[GeocodeFeatureAttributes])) => {
