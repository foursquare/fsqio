...
object StatsListener {
  ...
  def apply(period: Duration, collection: StatsCollection): StatsListener = {
    getOrRegister("period:%d".format(period.inMillis), collection, new LatchedStatsListener(collection, period, false))
  }
  ...
  def apply(period: Duration, collection: StatsCollection, filters: List[Regex]): StatsListener = {
    getOrRegister(
      "period:%d".format(period.inMillis),
      collection,
...
class StatsListener(collection: StatsCollection, startClean: Boolean, filters: List[Regex]) {
  ...
  if (startClean) {
    collection.getCounters().foreach {
      case (key, value) =>
        lastCounterMap(key) = value
    }
    collection.getMetrics().foreach {
      case (key, value) =>
  ...
  def get(): StatsSummary =
    StatsSummary(
      getCounters(),
      getMetrics().map({ case (key, histogram) => (key, Distribution(histogram)) }).toMap,
      getGauges(),
      getLabels()
    )
...
class LatchedStatsListener(
  ...
  def this(collection: StatsCollection, period: Duration, startClean: Boolean) =
    this(collection, period, startClean, Nil)
