...
import io.fsq.fhttpx.{FHttpClient, FHttpRequest, HttpStatusException, MultiPart, OAuth1Filter, Token}
...
class FHttpClientTest {
  ...
  def testOauthFlowGetPost(): Unit = {
    def testFlow(usePost: Boolean) {
      import io.fsq.fhttpx.FHttpRequest.asOAuth1Token
      val clientOA =
        new FHttpClient(
          "oauth",
