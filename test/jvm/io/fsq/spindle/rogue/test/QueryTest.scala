...
class QueryTest extends JUnitMustMatchers {
  ...
  def testProduceACorrectJSONQueryString {
    ...
    Q(ThriftVenue).where(_.id before d2).toString() must_== """db.venues.find({ "_id" : { "$lt" : ObjectId("%s")}})"""
      .format(oid2.toString)
    Q(ThriftVenue).where(_.id after d1).toString() must_== """db.venues.find({ "_id" : { "$gt" : ObjectId("%s")}})"""
    ...
    Q(ThriftVenue)
      ...
      .toString() must_== """db.venues.find({ "last_claim.uid" : 123})"""
    ...
    Q(ThriftVenue)
      ...
      .toString() must_== """db.venues.find({ "legid" : 1}, { "last_claim.status" : 1})"""
    ...
    Q(ThriftVenue)
      .where(_.mayor eqs 1)
      .limitOpt(Some(10))
      .toString() must_== """db.venues.find({ "mayor" : 1}).limit(10)"""
    ...
    Q(ThriftVenue)
      .where(_.mayor eqs 1)
      .skipOpt(Some(10))
      .toString() must_== """db.venues.find({ "mayor" : 1}).skip(10)"""
  ...
  def testModifyQueryShouldProduceACorrectJSONQueryString {
    ...
    Q(ThriftVenue)
      ...
      .toString() must_== query + """{ "$min" : { "last_updated" : { "$date" : "2010-05-01T00:00:00.000Z"}}}""" + suffix
    Q(ThriftVenue)
      .where(_.legacyid eqs 1)
      .modify(_.last_updated max d1)
      .toString() must_== query + """{ "$max" : { "last_updated" : { "$date" : "2010-05-01T00:00:00.000Z"}}}""" + suffix
    Q(ThriftVenue)
      .where(_.legacyid eqs 1)
    ...
    Q(ThriftVenue)
      ...
      .toString() must_== query + """{ "$set" : { "claims" : [ { "uid" : 1 , "status" : 1 , "source" : { "name" : "" , "url" : ""}}]}}""" + suffix
    Q(ThriftVenue)
      .where(_.legacyid eqs 1)
      .modify(_.lastClaim setTo claims.head)
      .toString() must_== query + """{ "$set" : { "last_claim" : { "uid" : 1 , "status" : 1 , "source" : { "name" : "" , "url" : ""}}}}""" + suffix
  ...
  def testWhereOpt {
    ...
    Q(ThriftVenue)
      ...
      .and(_.mayor eqs 2)
      .toString() must_== """db.venues.find({ "mayor" : 2})"""

    // whereOpt: lists
    Q(ThriftVenue)
      .whereOpt(someList)(_.legacyid in _)
      .toString() must_== """db.venues.find({ "legid" : { "$in" : [ 1 , 2]}})"""
