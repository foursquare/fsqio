...
class QueryTest extends JUnitMustMatchers {
  ...
  def testProduceACorrectJSONQueryString {
    ...
    Venue
      ...
      .toString() must_== """db.venues.find({ "legid" : { "$in" : [ 123 , 456]}})"""
    Venue
    ...
    Venue.where(_.categories at 0 eqs oid).toString() must_== """db.venues.find({ "categories.0" : ObjectId("%s")})"""
      .format(oid.toString)
    Venue
    ...
    Venue.where(_._id before d2).toString() must_== """db.venues.find({ "_id" : { "$lt" : ObjectId("%s")}})""".format(
      oid2.toString
    )
    Venue.where(_._id after d1).toString() must_== """db.venues.find({ "_id" : { "$gt" : ObjectId("%s")}})""".format(
      oid1.toString
    )
    Venue
      .where(_._id between (d1, d2))
      .toString() must_== """db.venues.find({ "_id" : { "$gt" : ObjectId("%s") , "$lt" : ObjectId("%s")}})""".format(
      oid1.toString,
      oid2.toString
    )
    Venue
    ...
    Venue
      ...
      .toString() must_== """db.venues.find({ "last_updated" : { "$lt" : { "$date" : "2010-05-02T00:00:00.000Z"}}})"""
    Venue
      .where(_.last_updated after d1)
      .toString() must_== """db.venues.find({ "last_updated" : { "$gt" : { "$date" : "2010-05-01T00:00:00.000Z"}}})"""
    Venue
      .where(_.last_updated between (d1, d2))
      .toString() must_== """db.venues.find({ "last_updated" : { "$gte" : { "$date" : "2010-05-01T00:00:00.000Z"} , "$lte" : { "$date" : "2010-05-02T00:00:00.000Z"}}})"""
    Venue
      .where(_.last_updated between Tuple2(d1, d2))
      .toString() must_== """db.venues.find({ "last_updated" : { "$gte" : { "$date" : "2010-05-01T00:00:00.000Z"} , "$lte" : { "$date" : "2010-05-02T00:00:00.000Z"}}})"""
    Venue
      .where(_.last_updated eqs d1)
      .toString() must_== """db.venues.find({ "last_updated" : { "$date" : "2010-05-01T00:00:00.000Z"}})"""
    Venue
    ...
    Venue
      ...
      .toString() must_== """db.venues.find({ "claims.uid" : { "$gte" : 1 , "$lte" : 10}})"""
    Venue
      .where(_.claims.subfield(_.date) between (d1.toDate, d2.toDate))
      .toString() must_== """db.venues.find({ "claims.date" : { "$gte" : { "$date" : "2010-05-01T00:00:00.000Z"} , "$lte" : { "$date" : "2010-05-02T00:00:00.000Z"}}})"""
    Venue
    ...
    Venue
      ...
      .toString() must_== """db.venues.find({ "mayor" : 1 , "tags" : "karaoke"})"""
    Venue
      .where(_.mayor eqs 1)
      .and(_.mayor_count eqs 5)
      .toString() must_== """db.venues.find({ "mayor" : 1 , "mayor_count" : 5})"""
    Venue
      .where(_.mayor eqs 1)
      .and(_.mayor_count lt 5)
      .toString() must_== """db.venues.find({ "mayor" : 1 , "mayor_count" : { "$lt" : 5}})"""
    Venue
      .where(_.mayor eqs 1)
      .and(_.mayor_count gt 3)
      .and(_.mayor_count lt 5)
      .toString() must_== """db.venues.find({ "mayor" : 1 , "mayor_count" : { "$lt" : 5 , "$gt" : 3}})"""
    ...
    Venue
      .where(_.mayor eqs 1)
      .orderAsc(_.legacyid)
      .toString() must_== """db.venues.find({ "mayor" : 1}).sort({ "legid" : 1})"""
    Venue
      .where(_.mayor eqs 1)
      .orderDesc(_.legacyid)
      .andAsc(_.userid)
      .toString() must_== """db.venues.find({ "mayor" : 1}).sort({ "legid" : -1 , "userid" : 1})"""
    Venue
      .where(_.mayor eqs 1)
      .orderDesc(_.lastClaim.subfield(_.date))
      .toString() must_== """db.venues.find({ "mayor" : 1}).sort({ "lastClaim.date" : -1})"""
    Venue
      .where(_.mayor eqs 1)
      .orderNaturalAsc
      .toString() must_== """db.venues.find({ "mayor" : 1}).sort({ "$natural" : 1})"""
    Venue
      .where(_.mayor eqs 1)
      .orderNaturalDesc
      .toString() must_== """db.venues.find({ "mayor" : 1}).sort({ "$natural" : -1})"""
    ...
    Venue
      .where(_.mayor eqs 1)
      .select(_.legacyid)
      .toString() must_== """db.venues.find({ "mayor" : 1}, { "legid" : 1})"""
    Venue
      .where(_.mayor eqs 1)
      .select(_.legacyid, _.userid)
      .toString() must_== """db.venues.find({ "mayor" : 1}, { "legid" : 1 , "userid" : 1})"""
    Venue
      .where(_.mayor eqs 1)
      .select(_.legacyid, _.userid, _.mayor)
      .toString() must_== """db.venues.find({ "mayor" : 1}, { "legid" : 1 , "userid" : 1 , "mayor" : 1})"""
    Venue
      .where(_.mayor eqs 1)
      .select(_.legacyid, _.userid, _.mayor, _.mayor_count)
      .toString() must_== """db.venues.find({ "mayor" : 1}, { "legid" : 1 , "userid" : 1 , "mayor" : 1 , "mayor_count" : 1})"""
    Venue
      .where(_.mayor eqs 1)
      .select(_.legacyid, _.userid, _.mayor, _.mayor_count, _.closed)
      .toString() must_== """db.venues.find({ "mayor" : 1}, { "legid" : 1 , "userid" : 1 , "mayor" : 1 , "mayor_count" : 1 , "closed" : 1})"""
    Venue
      .where(_.mayor eqs 1)
      .select(_.legacyid, _.userid, _.mayor, _.mayor_count, _.closed, _.tags)
      .toString() must_== """db.venues.find({ "mayor" : 1}, { "legid" : 1 , "userid" : 1 , "mayor" : 1 , "mayor_count" : 1 , "closed" : 1 , "tags" : 1})"""
    ...
    Tip
      ...
      .toString() must_== """db.tips.find({ "legid" : 1}, { "counts.foo" : 1})"""
    Venue
      .where(_.legacyid eqs 1)
      .select(_.geolatlng.unsafeField[Double]("lat"))
      .toString() must_== """db.venues.find({ "legid" : 1}, { "latlng.lat" : 1})"""
    Venue
      .where(_.legacyid eqs 1)
      .select(_.lastClaim.subselect(_.status))
      .toString() must_== """db.venues.find({ "legid" : 1}, { "lastClaim.status" : 1})"""
    Venue
      .where(_.legacyid eqs 1)
      .select(_.claims.subselect(_.userid))
      .toString() must_== """db.venues.find({ "legid" : 1}, { "claims.uid" : 1})"""
    ...
    Venue
      ...
      .toString() must_== """db.venues.find({ "legid" : 1}, { "tags" : { "$slice" : 4}})"""
    Venue
      .where(_.legacyid eqs 1)
      .select(_.tags.slice(4, 7))
      .toString() must_== """db.venues.find({ "legid" : 1}, { "tags" : { "$slice" : [ 4 , 7]}})"""

    // select $
    Venue
      .where(_.legacyid eqs 1)
      .select(_.tags.$$)
      .toString() must_== """db.venues.find({ "legid" : 1}, { "tags.$" : 1})"""
    ...
    Venue
      ...
      .toString() must_== """db.venues.find({ "legid" : 1 , "claims" : { "$elemMatch" : { "uid" : { "$gt" : 2097} , "status" : "Approved"}}})"""
    ...
    Venue
      .orderDesc(_._id)
      .and(_.mayor eqs 1)
      .toString() must_== """db.venues.find({ "mayor" : 1}).sort({ "_id" : -1})"""
    Venue
      .orderDesc(_._id)
      .skip(3)
      .and(_.mayor eqs 1)
      .toString() must_== """db.venues.find({ "mayor" : 1}).sort({ "_id" : -1}).skip(3)"""
    ...
    Venue
      ...
      .toString() must_== """db.venues.find({ "mayor" : 1 , "$where" : "this.a > 3"})"""

    // $not clauses
    Venue
      .scan(_.mayor eqs 1)
      .scan(_.mayor_count not (_ lt 5))
      .toString() must_== """db.venues.find({ "mayor" : 1 , "mayor_count" : { "$not" : { "$lt" : 5}}})"""
    Venue
      .scan(_.mayor eqs 1)
      .scan(_.mayor_count not (_ lt 5))
      .and(_.mayor_count not (_ gt 6))
      .toString() must_== """db.venues.find({ "mayor" : 1 , "mayor_count" : { "$not" : { "$gt" : 6 , "$lt" : 5}}})"""
    Venue
      .scan(_.mayor eqs 1)
      .scan(_.mayor_count not (_ lt 5))
      .and(_.mayor_count gt 3)
      .toString() must_== """db.venues.find({ "mayor" : 1 , "mayor_count" : { "$gt" : 3 , "$not" : { "$lt" : 5}}})"""
    Venue
      .scan(_._id not (_ before d1))
      .toString() must_== """db.venues.find({ "_id" : { "$not" : { "$lt" : ObjectId("%s")}}})""".format(oid1.toString)
    Venue
      .scan(_.last_updated not (_ between (d1, d2)))
      .toString() must_== """db.venues.find({ "last_updated" : { "$not" : { "$gte" : { "$date" : "2010-05-01T00:00:00.000Z"} , "$lte" : { "$date" : "2010-05-02T00:00:00.000Z"}}}})"""
    Venue
  ...
  def testModifyQueryShouldProduceACorrectJSONQueryString {
    ...
    Venue
      ...
      .toString() must_== query + """{ "$set" : { "last_updated" : { "$date" : "2010-05-01T00:00:00.000Z"}}}""" + suffix
    Venue
      .where(_.legacyid eqs 1)
    ...
    Venue
      ...
      .toString() must_== query + """{ "$push" : { "popularity" : { "$each" : [ 1 , 2]}}}""" + suffix
    Venue
      .where(_.legacyid eqs 1)
      .modify(_.popularity push (List(1L, 2L), 3))
      .toString() must_== query + """{ "$push" : { "popularity" : { "$each" : [ 1 , 2] , "$slice" : 3}}}""" + suffix
    Venue
      .where(_.legacyid eqs 1)
    ...
    Venue
      ...
      .toString() must_== query + """{ "$set" : { "claims" : [ { "status" : "Approved" , "uid" : 1 , "source" : { "name" : "" , "url" : ""} , "date" : { "$date" : "2010-05-01T00:00:00.000Z"}}]}}""" + suffix
    Venue
      .where(_.legacyid eqs 1)
      .modify(_.lastClaim setTo claims.head)
      .toString() must_== query + """{ "$set" : { "lastClaim" : { "status" : "Approved" , "uid" : 1 , "source" : { "name" : "" , "url" : ""} , "date" : { "$date" : "2010-05-01T00:00:00.000Z"}}}}""" + suffix
  ...
  def testOrQueryShouldProduceACorrectJSONQueryString {
    ...
    Venue
      ...
      .toString() must_== """db.venues.update({ "$or" : [ { "legid" : 1} , { "mayor" : 2}]}, { "$set" : { "userid" : 1}}, false, false)"""

    // $or with optional where clause
    Venue
      .or(_.where(_.legacyid eqs 1), _.whereOpt(None)(_.mayor eqs _))
      .modify(_.userid setTo 1)
      .toString() must_== """db.venues.update({ "$or" : [ { "legid" : 1}]}, { "$set" : { "userid" : 1}}, false, false)"""
  ...
  def testHints {
    Venue
      .where(_.legacyid eqs 1)
      .hint(Venue.idIdx)
      .toString() must_== """db.venues.find({ "legid" : 1}).hint({ "_id" : 1})"""
    Venue
      .where(_.legacyid eqs 1)
      .hint(Venue.legIdx)
      .toString() must_== """db.venues.find({ "legid" : 1}).hint({ "legid" : -1})"""
    Venue
      .where(_.legacyid eqs 1)
      .hint(Venue.geoIdx)
      .toString() must_== """db.venues.find({ "legid" : 1}).hint({ "latlng" : "2d"})"""
    Venue
      .where(_.legacyid eqs 1)
      .hint(Venue.geoCustomIdx)
      .toString() must_== """db.venues.find({ "legid" : 1}).hint({ "latlng" : "custom" , "tags" : 1})"""
  }
  ...
  def testWhereOpt {
    ...
    Venue
      ...
      .and(_.mayor eqs 2)
      .toString() must_== """db.venues.find({ "legid" : 1 , "mayor" : 2})"""
    ...
    Venue
      ...
      .toString() must_== """db.venues.find({ "mayor" : 2 , "legid" : 1})"""
  ...
  def testShardKey {
    ...
    Like
      ...
      .allShards
      .noop()
      .toString() must_== """db.likes.update({ "userid" : 123}, { }, false, false)"""
    Like.withShardKey(_.userid eqs 123).toString() must_== """db.likes.find({ "userid" : 123})"""
    Like
      .withShardKey(_.userid in List(123L, 456L))
      .toString() must_== """db.likes.find({ "userid" : { "$in" : [ 123 , 456]}})"""
    Like
      .withShardKey(_.userid eqs 123)
      .and(_.checkin eqs 1)
      .toString() must_== """db.likes.find({ "userid" : 123 , "checkin" : 1})"""
    Like
      .where(_.checkin eqs 1)
      .withShardKey(_.userid eqs 123)
      .toString() must_== """db.likes.find({ "checkin" : 1 , "userid" : 123})"""
  }
